...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if( 0 == size_ )
  {
    ...
    // update head node and tail node pointer
    head_ = new_node;
    tail_ = new_node;
    
    // new_node's next and prev has been set in Node's constructor
    
    
  }
  else if( newData <= head_->data )
  {
  // If LinkList is non-empty, and newData is the smallest
  
    // update new_node'next as _head (old)
    // update _head(old)'s prev as new_node
    new_node->next = head_;
    head_->prev = new_node;

    // new_node will be the head
    // tail head keep the same, no change    
    head_ = new_node;
  
  }
  else
  {
  // If LinkList is non-empty, and newData is not the smallest
    ...
    while( nullptr != current )
    {
      ...
      if( current->data <= newData )
      {
        ...
        // If current->next exists (non-empty)
        if( nullptr != current->next  )
        {
          ...
          // Current moves on next one
          current = current->next;           
        }
        else
        {
        // If current->next is nullptr (the last one [tail node] will be new_node)  
          
          // update current's next and new_node's prev
          current->next=new_node;
          new_node->prev = current;
          
          // update tail node pointer
          // head node keep the same ( no change)
          tail_ = new_node;
          
          break;
        }
        //end of if nullptr != current->next ... else ...

      }
      else
      {
        // on the correct position with increasing order
        
        // prev's next is new_node
        // new_node's prev = pointer prev
        prev->next = new_node;
        new_node->prev = prev;
        
        // new_node's next = current
        // current's prev = new_node
        new_node->next = current;
        current->prev = new_node;
        
        // new node is placed in the middle of linked list,
        // so head node and tail node keep the same (no change)
        
        break;
      }
      //end of if current->data <= newData ... else ...
    }
    //end of while loop
    
  }
  //end of if 0 == size_  ... else ...
  ...
  // Don't forget to update the list size and the head_ and tail_
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  //end of while loop: this is not empty and other is not empty
  
  if( left.empty() )
  {
    while( false == right.empty() )
    {
      // If left-halve is empty, add all remaining one by one from right halves to merge list.
      merged.pushBack( right.front() );
      right.popFront();
    }
  }
  //end of if *this is empty
  
  if( right.empty() )
  {
    while( false == left.empty() )
    {
      // If right-halve is empty, add all remaining one by one from left halve to merge list.
      merged.pushBack( left.front() );
      left.popFront();
    }
  }
  //end of if other is empty
